...
class Config:
    ...
    # --- Combat tuning ---
    # Base win probability when attacker and defender are equal strength.
    # e.g. 0.6 means the attacker wins 60% of even fights.
    base_attack_win_chance: float = 0.6
 
    # How strongly territory size influences combat.
    # Higher = larger countries have a bigger advantage.
    # At 1.0 a country with 2x the tiles has ~2x the combat strength.
    strength_scaling_power: float = 1.0
 
    # Minimum and maximum win-probability clamp, so fights are never trivial
    # and never completely hopeless for the underdog.
    min_win_chance: float = 0.1
    max_win_chance: float = 0.9
...
# ---------------------------------------------------------------------------
# Strength / combat helpers  (NEW)
# ---------------------------------------------------------------------------
 
def get_territory_counts(state: GameState) -> dict:
    """Return {country_id: territory_count} for all non-terrain cells."""
    return Counter(
        v for v in state.flat_mat
        if v not in (Tile.MOUNTAIN, Tile.WATER)
    )
 
 
def compute_strength(territory_count: int, cfg: Config) -> float:
    """
    Convert a territory count into a combat strength value.
    Uses a power curve so larger countries are stronger, but not linearly so —
    this keeps smaller countries competitive.
    """
    return max(1, territory_count) ** cfg.strength_scaling_power
 
 
def attack_succeeds(attacker_id: int, defender_id: int, state: GameState) -> bool:
    """
    Roll for combat outcome.
 
    Win probability is derived from the ratio of attacker strength to total
    combined strength, then scaled around the base_attack_win_chance so that
    an equal fight uses that base value rather than 50/50.
 
    Formula:
        raw_ratio  = attacker_strength / (attacker_strength + defender_strength)
        # raw_ratio == 0.5 when equal; shift it so 0.5 maps to base_chance
        win_chance = base_chance + (raw_ratio - 0.5) * 2 * (1 - base_chance)
        win_chance = clamp(win_chance, min_chance, max_chance)
    """
    cfg = state.cfg
    counts = get_territory_counts(state)
 
    att_strength = compute_strength(counts.get(attacker_id, 1), cfg)
    def_strength = compute_strength(counts.get(defender_id, 1), cfg)
 
    raw_ratio = att_strength / (att_strength + def_strength)
 
    # Scale so equal strength → base_attack_win_chance
    base = cfg.base_attack_win_chance
    win_chance = base + (raw_ratio - 0.5) * 2 * (1 - base)
    win_chance = max(cfg.min_win_chance, min(cfg.max_win_chance, win_chance))
...
# ---------------------------------------------------------------------------
# Combat  (UPDATED — randomised outcome + strength scaling)
# ---------------------------------------------------------------------------
...
def resolve_combat(attacker_id: int, x: int, y: int, state: GameState) -> None:
    ...
    The attack is then resolved probabilistically using strength scaling.
    ...
    for _ in range(max_loops):
        for pos in attacker_frontier:
            ...
            # Valid target found — roll for combat outcome
            defender_id = neighbour_val
            if attack_succeeds(attacker_id, defender_id, state):
                _apply_attack(attacker_id, defender_id, ny, nx, x, y, state)
            else:
                _print_repelled(attacker_id, defender_id, state.move)
            return
...
def _apply_attack(attacker_id: int, defender_id: int,
    ...
    """Apply the result of a successful attack."""
    cfg = state.cfg
...
def _print_repelled(attacker_id: int, defender_id: int, move: int) -> None:
    """Print a message when an attack is repelled."""
    print(f'Move {move}: {attacker_id} attacked {defender_id} but was repelled!')
