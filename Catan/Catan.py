...
class Catan:
    ...
    def __init__(self):
        ...
        # Team LIST
        self.team_mat.append(TEAM_1)
        self.team_mat.append(TEAM_2)
        self.team_mat.append(TEAM_3)

        game = Game(self.team_mat)

        random.shuffle(C.DEVELOPMENT_CARD_DECK)

        # Select Positions
        for pre_turn in range(4 * len(self.team_mat)):
            ...
            if pre_turn in [0, 5, 6, 11]:
                ...
                C.selected_team = self.team_mat[0]
            ...
            # Team 2
                ...
                C.selected_team = self.team_mat[1]

                if self.team_mat[1].is_human == 0:

                    game.intial_ai_build(self.team_mat[1])
            ...
            # Team 2
                ...
                C.selected_team = self.team_mat[2]

                if self.team_mat[2].is_human == 0:

                    game.intial_ai_build(self.team_mat[2])
        ...
        def isterminal():
            ...
            if win_con < C.NO_OF_VICTORY_WINNING_POINTS:
                return True
            else:
                return False
        ...
        while not isterminal():
        ...
        #if self.team_mat[turn % C.NO_OF_PLAYER].is_human == 1:
            ...
            if dice_roll == 7:
                ...
                if self.team_mat[turn % C.NO_OF_PLAYER].is_human == 1:
                    ...
                    game.ai_robber(self.team_mat[turn % C.NO_OF_PLAYER])
            ...
            while C.selecting_type != 'End Turn':
                ...
                ava_resources = self.team_mat[turn % C.NO_OF_PLAYER].resources
                ...
                if count_wood >= C.BUILD_ROAD_REQ_WOOD and count_brick >= C.BUILD_ROAD_REQ_BRICK and \
                    self.team_mat[turn % C.NO_OF_PLAYER].avaliable_road_pieces > 0 and possible_road == 1:
                ...
                if count_wood >= C.BUILD_SETTLEMENT_REQ_WOOD and \
                    ...
                    count_wheat >= C.BUILD_SETTLEMENT_REQ_WHEAT and \
                        count_sheep >= C.BUILD_SETTLEMENT_REQ_SHEEP and \
                            self.team_mat[turn % C.NO_OF_PLAYER].avaliable_settlement_pieces > 0 and \
                                possible_settlement == 1:
                ...
                if count_wheat >= C.BUILD_CITY_REQ_WHEAT and count_stone >= C.BUILD_CITY_REQ_STONE and \
                    self.team_mat[turn % C.NO_OF_PLAYER].avaliable_city_pieces > 0 and possible_city == 1:
                ...
                if len(self.team_mat[turn % C.NO_OF_PLAYER].cards) > self.team_mat[turn % C.NO_OF_PLAYER].cards.count('VP') and possible_play_card == 1:
                ...
                game_trade_check = 0
                # Trade with Game
                counts = Counter(self.team_mat[turn % C.NO_OF_PLAYER].resources)
                if len(self.team_mat[turn % C.NO_OF_PLAYER].resources) > 1:
                ...
                for i in game.board.seaports_class:
                    for j in i.corner_mat_class:
                        if j.is_occupied == 1 and j.team == self.team_mat[turn % C.NO_OF_PLAYER].team and j.seaport_trade == 1:
                            if i.number == -3:
                                if occurrences >= 3:
                                    seaport_trade_check = 1
                            elif i.number == -2:
                                counts = self.team_mat[turn % C.NO_OF_PLAYER].resources.count(i.tile_type)
                                if counts > 2:
                ...
                if C.selecting_type in ('Settlement', 'Road', 'City'):
                    ...
                    if self.team_mat[turn % C.NO_OF_PLAYER].is_human == 1:
                        C.selecting_on_screen = False
                        C.selected_team = self.team_mat[turn % C.NO_OF_PLAYER]
                        ...
                        if C.selecting_type == 'Settlement':
                            if game.ai_find_settlement(self.team_mat[turn % C.NO_OF_PLAYER]):
                                C.built = 'Settlement'
                            else:
                                possible_settlement = 0

                        elif C.selecting_type == 'Road':
                            if game.ai_find_road(self.team_mat[turn % C.NO_OF_PLAYER]):
                                C.built = 'Road'
                            else:
                                possible_road = 0

                        elif C.selecting_type == 'City':
                            if game.ai_build_city(self.team_mat[turn % C.NO_OF_PLAYER]):
                                C.built = 'City'
                            else:
                                possible_city = 0
                    ...
                    if C.built == 'Settlement':
                        self.team_mat[turn % C.NO_OF_PLAYER].current_points += 1
                        self.team_mat[turn % C.NO_OF_PLAYER].avaliable_settlement_pieces -= 1
                        
                        # Remove Resources
                        for _ in range(C.BUILD_SETTLEMENT_REQ_WOOD):
                            self.team_mat[turn % C.NO_OF_PLAYER].resources.remove('Wood')

                        for _ in range(C.BUILD_SETTLEMENT_REQ_BRICK):
                            self.team_mat[turn % C.NO_OF_PLAYER].resources.remove('Brick')

                        for _ in range(C.BUILD_SETTLEMENT_REQ_WHEAT):
                            self.team_mat[turn % C.NO_OF_PLAYER].resources.remove('Wheat')

                        for _ in range(C.BUILD_SETTLEMENT_REQ_SHEEP):
                            self.team_mat[turn % C.NO_OF_PLAYER].resources.remove('Sheep')

                    elif C.built == 'City':
                        self.team_mat[turn % C.NO_OF_PLAYER].current_points += 1
                        self.team_mat[turn % C.NO_OF_PLAYER].avaliable_city_pieces -= 1
                        self.team_mat[turn % C.NO_OF_PLAYER].avaliable_settlement_pieces += 1
                        ...
                        for _ in range(C.BUILD_CITY_REQ_STONE):
                            self.team_mat[turn % C.NO_OF_PLAYER].resources.remove('Stone')

                        C.main_turtle.clear()
                        game.board.draw_current_board(self.team_mat)

                    elif C.built == 'Road':

                        self.team_mat[turn % C.NO_OF_PLAYER].avaliable_road_pieces -= 1
                    ...
                    self.team_mat[turn % C.NO_OF_PLAYER].cards.append(C.DEVELOPMENT_CARD_DECK[0])
                    ...
                    if C.selecting_type == 'Knight':
                        ...
                        if self.team_mat[turn % C.NO_OF_PLAYER].is_human == 1:
                            ...
                            self.team_mat[turn % C.NO_OF_PLAYER].cards.remove('Knight')

                            self.team_mat[turn % C.NO_OF_PLAYER].played_knights += 1
                        ...
                        if C.selecting_type != 'End Turn':
                            ...
                            for i in self.team_mat:
                                ...
                                # Skip Itself
                                if i == self.team_mat[turn % C.NO_OF_PLAYER]:
                                    continue
                                ...
                                if C.selecting_type in i.resources:
                                    ...
                                    # Add to player
                                    self.team_mat[turn % C.NO_OF_PLAYER].resources.extend([C.selecting_type] * occurrence)

                                    for _ in range(i.resources.count(C.selecting_type)):
                                        i.resources.remove(C.selecting_type)

                            self.team_mat[turn % C.NO_OF_PLAYER].cards.remove('Monopoly')
                        ...
                        if C.selecting_type != 'End Turn':
                            self.team_mat[turn % C.NO_OF_PLAYER].cards.remove('Road Building')
                        ...
                        for i in range(2):
                            ...
                            if C.selecting_type != 'End Turn':
                                self.team_mat[turn % C.NO_OF_PLAYER].resources.append(C.selecting_type)
                        ...
                        self.team_mat[turn % C.NO_OF_PLAYER].cards.remove('Year of Plenty')
                    ...
                    for _ in range(number_of_trades):
                        self.team_mat[turn % C.NO_OF_PLAYER].resources.remove(original_resource)

                    self.team_mat[turn % C.NO_OF_PLAYER].resources.append(new_resource)
...
@torch.no_grad()
def choose_move(main_game: Catan, model: nn.Module, epsilon=0.2):
    moves = main_game.legal_moves()
    if random.random() < epsilon:
        return random.choice(moves)

    # Pick the move that leads to the highest predicted value for the NEXT player-to-move state
    # (After we play a move, turn switches. encode_state() always encodes from to-move POV,
    ...
    for m in moves:
        g2 = main_game.clone()
        g2.play(m)
        x2 = main_game.game.encode_state(g2).unsqueeze(0)
        v2 = model(x2).item()
        # v2 is from g2's player-to-move POV (the opponent relative to current state),
        # so from CURRENT player's POV it's -v2.
        v_current_pov = -v2
        if v_current_pov > best_val:
            best_val = v_current_pov
            best_move = m
